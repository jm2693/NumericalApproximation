...
def bisectionMethod(f, a, b, epsilon=1e-6, max_iterations=100, result=None):
    ...
    if abs(fc) < epsilon:
        return result.append((len(result) + 1, a, b, c, fc))
...
def newtonMethod(f, df, x0, epsilon=1e-6, max_iterations=100, result=None):
    ...
    if fx0 < epsilon:
        return result.append((len(result) + 1, x0, fx0))
    result.append((len(result) + 1, x0, fx0))
    
    newtonMethod(f, df, x0, epsilon, max_iterations-1, result)
...
def secantMethod(f, x0, x1, epsilon=1e-6, max_iterations=100, result=None):
    ...
    if fx2 < epsilon:
        return result.append((len(result) + 1, x0, x1, x2, fx2))
    result.append((len(result) + 1, x0, x1, x2, fx2))
    
    secantMethod(f, x1, x2, epsilon, max_iterations - 1, result)
